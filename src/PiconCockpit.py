...
from .ChannelListUtils import getServiceList, getTVBouquets, getRadioBouquets
from .WebRequestsAsync import WebRequestsAsync
from .Debug import logger
from .__init__ import _
from .FileUtils import readFile, createDirectory, deleteFiles
from .SetupScreen import SetupScreen
from .PiconDownloadProgress import PiconDownloadProgress
from .ConfigInit import updateFilterChoices
...
class PiconCockpit(Screen):
    ...
    def __onLayoutFinish(self):
        ...
        if self.onSelectionChanged not in self["list"].onSelectionChanged:
    ...
    def getPiconSetInfo(self):
        ...
        try:
            ...
            url = urljoin(base_url, picon_info_file)
    ...
    def downloadError(self, failure, url=None):
        ...
        self.showErrorMessage(_("Picon server access failed"))
        self.createList(False)
    ...
    def openSetupScreenCallback(self, _result=None):
        logger.info("...")
        self.first_start = True
    ...
    def exit(self):
        ...
        deleteFiles(os.path.join("/tmp", "*.png"))
        self.close()
    ...
    def green(self):
        ...
        try:
            if picon_set:
                ...
                try:
                    picon_set_url = picon_set[1]
                    if not picon_set_url.endswith('/'):
                        picon_set_url += '/'

                    url = urljoin(picon_set_url, picon_list_file)
                    ...
                    downloader = self.web_client.downloadFileAsync(url, download_file)
                    downloader.addCallback(lambda result: self.downloadPicons(result, picon_set))
    ...
    def getUserBouquetPicons(self):
        ...
        try:
            ...
            logger.error("Error in getUserBouquetPicons: %s", e)
            return []
        ...
        if answer:
            try:
                ...
                if picons:
                    picon_set = self["list"].getCurrent()
                    if picon_set:
                        if config.plugins.piconcockpit.delete_before_download.value:
                            deleteFiles(os.path.join(self.picon_dir, "*.png"))
    ...
    def parseSettingsOptions(self, picon_set_list):
        ...
        if picon_set_list:
            updateFilterChoices(list(size_list), list(bit_list),
    ...
    def parsePiconSetList(self, picon_set_list):
        ...
        for picon_set in picon_set_list:
            if not picon_set.startswith('<meta'):
                info_list = picon_set.split(';')
                if len(info_list) >= 9:
                    server = config.plugins.piconcockpit.picon_server.value.rstrip('/')
                    dir_url = f"{server}/{info_list[0]}"
                    pic_url = f"{server}/{info_list[0]}/{info_list[1]}"
                    date = info_list[2]
                    name = info_list[3]
